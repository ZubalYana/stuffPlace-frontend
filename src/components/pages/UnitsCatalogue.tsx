			...
			<Box className='w-full mt-6 flex justify-end'>
				<Stack direction='row' spacing={3} alignItems='center' flexWrap='wrap'>
					<FormControl variant='standard' size='small'>
						...
						<Select
							value={capacity}
							onChange={(e) => setCapacity((e.target.value as number) || '')}
							disableUnderline
							displayEmpty
							MenuProps={{
						...
						>
							...
							{[1, 2, 4, 6, 8].map((n) => (
								<MenuItem key={n} value={n}>
									{n}+{' '}
									{localStorage.getItem('language') === 'en' ? 'people' : 'fős'}
								</MenuItem>
							))}
						</Select>
					</FormControl>
					...
					<FormControl variant='standard' size='small'>
						...
						>
							...
							<MenuItem value='Single'>
								{localStorage.getItem('language') === 'en' ? 'Single' : '1 fős'}
							</MenuItem>
							<MenuItem value='2-Person'>
								{localStorage.getItem('language') === 'en'
									? '2-Person'
									: '2 fős'}
							</MenuItem>
							<MenuItem value='3-Person'>
								{localStorage.getItem('language') === 'en'
									? '3-Person'
									: '3 fős'}
							</MenuItem>
							<MenuItem value='4-Person'>
								{localStorage.getItem('language') === 'en'
									? '4-Person'
									: '4 fős'}
							</MenuItem>
							<MenuItem value='6-Person'>
								{localStorage.getItem('language') === 'en'
									? '6-Person'
									: '6 fős'}
							</MenuItem>
							<MenuItem value='8-Person'>
								{localStorage.getItem('language') === 'en'
									? '8-Person'
									: '8 fős'}
							</MenuItem>
