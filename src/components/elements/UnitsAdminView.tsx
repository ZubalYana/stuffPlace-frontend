...
export const UnitsAdminView = ({
	...
	const toggleHighlight = async (e: React.MouseEvent, unitId: string) => {
		e.stopPropagation()
		const res = await fetch(`${API_URL}/units/${unitId}/highlight`, {
			method: 'PATCH',
		})
	...
	return (
		<div className='w-full h-full flex flex-col mt-10 lg:mt-0'>
			...
			<div className='w-full h-screen md:h-[440px] overflow-y-auto custom-scroll'>
							...
							<div className='mt-4 lg:max-xl:mt-4 md:mt-0 md:ml-4 lg:max-xl:ml-0'>
								<div className='flex flex-col md:flex-row md:gap-4 gap-2 mb-2'>
									<p className='font-light text-[14px]'>
										Occupancy:{' '}
										<span className='font-semibold'>{unit.occupancy}</span>
									</p>
									<p className='font-light text-[14px]'>
